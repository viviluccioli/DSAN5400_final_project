---
title: "Media Tone Analysis: MSNBC News Coverage of U.S. Elections"
author: "Ria Sonawane"
format: 
  html:
...
execute:
  jupyter: python3
  python: "/opt/anaconda3/envs/dsan5400/bin/python"
  notebook-output: false
---
...
The dataset contains Fox News coverage from GDELT's database, including articles from five election cycles:
...
csv_files = glob.glob("../data/msnbc/msnbc*.csv")
...
plt.title("Distribution of MSNBC News Tone Scores", fontsize=14, fontweight='bold')
...
Note: GDELT tone scores typically range from -10 (extremely negative) to +10 (extremely positive), but most news content falls between -5 and +1. MSNBC'S range falls between -27 and 20. MSNBC News coverage has a mean tone around -2.7, reflecting the generally negative tone common in news media.
...
plt.title("MSNBC News Coverage Tone Over Time", fontsize=16, fontweight='bold')
...
The visualization reveals temporal patterns in Fox News coverage tone:
...
- Overall, tone scores remain primarily negative throughout the period, consistent with media's tendency to focus on problems and conflicts. Other than a large spike around 2018, there is a decreasing trend throughout the time horizon. 
...
plt.title("Top 20 Themes in MSNBC News Coverage", fontsize=16, fontweight='bold')
...
The visualization shows MSNBC News' dominant themes across the full dataset period:

- **Political Focus**: Presidential coverage, leadership, and general politics dominate
- **Partisan Focus**: A consistently significant theme in MSNBC News coverage
- **Other Notable Themes**: Political processes, partisan tax/policy, justice and governance, and broader social issues
...
plt.suptitle("Pre-Election Media Focus: MSNBC News Themes Before Each Election", 
...
- Partisan focus and social issues appear consistently across multiple election cycles
...
plt.suptitle("Post-Election Media Focus: MSNBC News Themes After Each Election", 
             fontsize=20, y=1.02, fontweight='bold')
...
- Government administration policies become more prominent in the post-election period
...
- **Rows (Y-axis)**: Each theme extracted from MSNBC News coverage
...
key_themes = ['TAX_POLITICAL_PARTY_REPUBLICAN', 'General Politics']
...
Our analysis of MSNBC News coverage across five election cycles reveals several significant patterns:
...
3. **Consistent Themes**: Presidential leadership, political parties, and general government operations persist as dominant themes across all periods.
...
- Comparing MSNBC News with other media outlets
...
- Tracking international or domestic shifts
