...
::: {.column width="33%"}
![](images/fox.png){width=45%}
:::

::: {.column width="33%"}
![](images/abc.png){width=70%}
:::

::: {.column width="33%"}
![](images/msnbc.png){width=55%}
:::
...
# Starting with the calculation of monthly averages
...
fig.add_shape(
    type="line",
    x0=rolling_tone.index.min(),
    x1=rolling_tone.index.max(),
    y0=0,
    y1=0,
    line=dict(
        color="black",
...
y_min = rolling_tone.min().min() - 0.5
y_max = rolling_tone.max().max() + 0.5

# Create frames for the animation
for i in range(num_frames):
    frame_data = []
    # Calculate how much of the data to show in this frame
    cutoff_idx = int((i + 1) * len(rolling_tone) / num_frames)
    
    # If we're at the last frame, make sure we show all data
    if i == num_frames - 1:
        cutoff_idx = len(rolling_tone)
    
    # Create a trace for each network
    for column in rolling_tone.columns:
        visible_data = rolling_tone[column].iloc[:cutoff_idx]
        
        frame_data.append(
            go.Scatter(
                x=visible_data.index,
                y=visible_data.values,
                mode="lines",
                name=column,
...
for column in rolling_tone.columns:
    fig.add_trace(
        go.Scatter(
            x=[rolling_tone.index[0]],  # Start with just the first point
            y=[rolling_tone[column].iloc[0]],
            mode="lines",
            name=column,
            line=dict(color=NETWORK_COLORS[column], width=4)
        )
...
fig.update_layout(
    title=dict(
        text=f"{rolling_window}-Month Rolling Average Tone by News Network (2015-2025)",
    ...
    annotations=[
        dict(
            text="Negative values indicate negative sentiment, positive values indicate positive sentiment",
            showarrow=False,
            xref="paper",
            yref="paper",
            x=0.5,
