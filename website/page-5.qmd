...
def preprocess_combined_df(df: pd.DataFrame) -> pd.DataFrame:
    """Clean and structure the dataset for topic half-life modeling."""
    ...
    df = df.loc[:, columns].copy()  # <-- Ensures this is a new, independent DataFrame

    # Remove trailing ' UTC' and strip whitespace
    df["parsed_date"] = (
        df["parsed_date"]
        .astype(str)
    ...
    # Process themes: split and explode into one row per theme
...
def estimate_half_life(topic_daily: pd.DataFrame, min_obs: int = 2, min_lambda: float = 1e-5) -> pd.DataFrame:
    """Estimate topic half-life via exponential decay for each source-theme pair."""
    from sklearn.linear_model import LinearRegression
    import numpy as np
    import pandas as pd
    ...
    for (source, topic), group in topic_daily.groupby(["source", "V2Themes"]):
        ...
        if lambda_ <= 0 or lambda_ < min_lambda:
            continue
