...
# Correct version: don't overwrite df
for media in media_sources:
    for year in years:
        path = f"../data/{media}/{media}{year}.csv"
        if os.path.exists(path):
            df_temp = pd.read_csv(path, on_bad_lines="skip")  # <- use df_temp
            df_temp["media"] = media
...
sns.boxplot(data=polar_agg[polar_agg["year"] == 2016], x="media", y="polarization", palette="Set2")
...
# Now filter safely
df_window = df[(df['parsed_date'] >= start_date) & (df['parsed_date'] <= end_date)].copy()

# Create a column for 5-day periods
df_window['days_since_start'] = (df_window['parsed_date'] - start_date).dt.days
df_window['period_num'] = df_window['days_since_start'] // 5
period_start_dates = [start_date + pd.Timedelta(days=i*5) for i in range(13)]  # 12 periods
...
fig.update_layout(
    ...
    xaxis=dict(
        title="Date (Oct 4 - Dec 3, 2020)",
        titlefont=dict(size=14),
        gridcolor="#E5E5E5",
        showgrid=True,
        tickformat="%b %d",
        dtick="M5"  # Tick about every 5 days
    ),
    yaxis=dict(
        title="Average Tone Score",
        titlefont=dict(size=14),
...
```{python}
plt.figure(figsize=(8, 5))
sns.boxplot(data=polar_agg[polar_agg["year"] == 2020], x="media", y="polarization", palette="Set2")
...
```{python}
plt.figure(figsize=(8, 5))
sns.boxplot(data=polar_agg[polar_agg["year"] == 2024], x="media", y="polarization", palette="Set2")
