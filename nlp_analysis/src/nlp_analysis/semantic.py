...
import logging
import pandas as pd
from nlp_analysis.utils import run_sentiment

logging.basicConfig(
    filename="../logs/tone_analysis.log",
    level=logging.INFO,
...
def analyze_year(year, election_day):
    ...
    for media in df_filtered["media"].unique():
        ...
        if os.path.exists(file_path):
            ...
            vader_sentiment_map = dict(
                zip(df_media["url"], df_media["vader_sentiment_analysis"])
            )
            ...
            original_df["vader_sentiment_analysis"] = original_df["url"].map(
                vader_sentiment_map
            )
...
if __name__ == "__main__":
    elections = {2016: "2016-11-08", 2020: "2020-11-03", 2024: "2024-11-05"}
