...
def normalize_topic_label(topic_words: List[str]) -> str:
    ...
    filtered_words = [
        ...
        if w.lower() not in ["html", "http", "href", "css", "src", "div", "span"]
    ]
...
def plot_top_themes_by_year(
    df: pd.DataFrame, title: str, filename: str, min_count: int = 2
) -> None:
    """
    ...
    significant_topics = significant_topics[
        significant_topics >= min_count
    ].index.tolist()

    # Get top 10 topics by total count
    top_topics = (
        yearly_topics.groupby("topic")["count"].sum().nlargest(10).index.tolist()
    )

    # Use intersection of significant and top topics
    topics_to_use = [t for t in top_topics if t in significant_topics][
        :10
    ]  # Limit to 10
    ...
    pivot_data = plot_data.pivot(index="topic", columns="year", values="count").fillna(
        0
    )
...
def main():
            ...
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    if "parsed_date" in df.columns:
                        ...
                        print(
                            f"No 'parsed_date' column in {file_path}, trying to extract from DATE"
                        )
                    ...
                    if "headline_from_url" in df.columns:
                        df["headline_from_url"] = (
                            df["headline_from_url"]
                            .astype(str)
                            .apply(lambda x: x if x != "nan" else "")
                        )
    ...
    for source in news_sources:
        ...
        if all_theme_counts:
            ...
            v2themes_results[source]["percentage"] = (
                v2themes_results[source]["count"]
                / v2themes_results[source]["total_articles"]
            ) * 100
        else:
    ...
    for source in news_sources:
        ...
        for year in tqdm(years_available, desc=f"Processing {source} years"):
            ...
            if "headline_from_url" not in df.columns:
                print(
                    f"No headline_from_url column in {source} data for {year}, skipping"
                )
                continue
            ...
            for quarter, months in quarters.items():
                ...
                if len(quarter_data) < 50:  # Need enough data for meaningful topics
                    print(
                        f"Skipping {source} {year} Q{quarter}: insufficient headlines ({len(quarter_data)})"
                    )
                    continue
                ...
                if len(headlines) < 50:  # Further check after filtering
                    print(
                        f"Skipping {source} {year} Q{quarter}: insufficient valid headlines ({len(headlines)})"
                    )
                    continue
                ...
                try:
                    ...
                    doc_topics, topic_words = sklearn_topic_modeling(
                        headlines, num_topics=num_topics
                    )
                    ...
                    print(
                        f"Error in topic modeling for {source} {year} Q{quarter}: {str(e)}"
                    )
                    continue
    ...
    for source in news_sources:
        ...
        if source in headline_results:
            output_file = os.path.join(
                output_data_dir, f"{source}_headline_topics_sklearn.csv"
            )
    ...
    for source in news_sources:
        ...
        if source in headline_results and not headline_results[source].empty:
            try:
                ...
                print(
                    f"Error creating visualization for {source} headline topics: {str(e)}"
                )
    ...
    for source in news_sources:
        ...
        if source in v2themes_results:
            ...
            if not v2themes_results[source].empty:
                top_themes = (
                    v2themes_results[source][v2themes_results[source]["rank"] == 1][
                        "topic"
                    ]
                    .value_counts()
                    .head(3)
                ...
                if not top_themes.empty:
                    print(
                        f"  - Most common top themes: {', '.join(top_themes.index.tolist())}"
                    )
        ...
        if source in headline_results:
            ...
            if not headline_results[source].empty:
                top_headline_topics = (
                    headline_results[source][headline_results[source]["rank"] == 1][
                        "topic"
                    ]
                    .value_counts()
                    .head(3)
                ...
                if not top_headline_topics.empty:
                    print(
                        f"  - Most common headline topics: {', '.join(top_headline_topics.index.tolist())}"
                    )
