...
Place this in your nlp_analysis directory and run directly with:
$ poetry run python topic_modeling.py
"""
...
output_dir = os.path.join(current_dir, 'results', 'topic_modeling')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(project_dir, 'logs', 'topic_modeling.log'), mode='w'),
...
def plot_top_themes_by_year(df: pd.DataFrame, title: str, filename: str, output_dir: str) -> None:
    """
    ...
    # Save figure
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'figures', filename), dpi=300)
    plt.close()
...
def main():
    ...
    for source in news_sources:
        for year in years:
            ...
            if os.path.exists(file_path):
                logger.info(f"Loading {source} data for {year}...")
                try:
                    ...
                    if 'parsed_date' in df.columns:
                        ...
                        logger.warning(f"No 'parsed_date' column in {file_path}, trying to extract from DATE")
    ...
    logger.info("Extracting V2Themes topics...")
    v2themes_results = {}
    
    for source in news_sources:
        logger.info(f"Extracting V2Themes for {source}...")
        # Create a list to store all theme counts
        all_theme_counts = []
        
        years_available = list(datasets[source].keys())
        if not years_available:
            logger.warning(f"No data available for {source}")
            continue
        ...
        for year in years_available:
            ...
            if 'V2Themes' not in df.columns:
                logger.warning(f"No V2Themes column in {source} data for {year}, skipping")
                continue
    ...
    logger.info("Modeling headline topics...")
    headline_results = {}
    
    for source in news_sources:
        logger.info(f"Modeling headline topics for {source}...")
        ...
        for year in years_available:
            ...
            if 'headline_from_url' not in df.columns:
                logger.warning(f"No headline_from_url column in {source} data for {year}, skipping")
                continue
            ...
            for (yr, mn), group in grouped:
                ...
                if len(headlines) < 10:  # Skip if too few headlines
                    logger.warning(f"Skipping {source} {yr}-{mn}: insufficient headlines ({len(headlines)})")
                    continue
                ...
                try:
                    ...
                    logger.error(f"Error in topic modeling for {source} {yr}-{mn}: {str(e)}")
                    continue
    ...
    for source in news_sources:
        if source in v2themes_results:
            output_file = os.path.join(output_dir, f"{source}_v2themes_topics.csv")
            logger.info(f"Saving V2Themes results to {output_file}")
            v2themes_results[source].to_csv(output_file, index=False)
        
        if source in headline_results:
            output_file = os.path.join(output_dir, f"{source}_headline_topics.csv")
            logger.info(f"Saving headline topics to {output_file}")
    ...
    if all_v2themes:
        ...
        combined_v2themes.to_csv(
            os.path.join(output_dir, "all_sources_v2themes_topics.csv"),
            index=False
        )
    ...
    if all_headlines:
        ...
        combined_headlines.to_csv(
            os.path.join(output_dir, "all_sources_headline_topics.csv"),
            index=False
        )
    ...
    for source in news_sources:
        if source in v2themes_results and not v2themes_results[source].empty:
            try:
                ...
                logger.error(f"Error creating visualization for {source} V2Themes: {str(e)}")
        ...
        if source in headline_results and not headline_results[source].empty:
            try:
                ...
                logger.error(f"Error creating visualization for {source} headline topics: {str(e)}")
