...
def analyze_election_sentiment(election_year):
    ...
    for network in ["fox", "abc", "msnbc"]:
        ...
        if not files:
            logging.warning(
                f"No {network} files found for {election_year} with pattern {pattern}"
            )
                ...
                if len(election_period_df) > 0:
                    ...
                    if "url" in processed_df.columns:
                        for _, row in processed_df.iterrows():
                            url = row["url"]
                            sentiment_map[url] = {
                                "vader_sentiment_analysis": row.get(
                        ...
                        for idx, row in df.iterrows():
                            if "url" in row and row["url"] in sentiment_map:
                                df.at[idx, "vader_sentiment_analysis"] = sentiment_map[
                                    row["url"]
                                ]["vader_sentiment_analysis"]
                                df.at[idx, "vader_tone_score"] = sentiment_map[
                                    row["url"]
                                ]["vader_tone_score"]
    ...
    logging.info(
        f"Before election: {len(combined_df[combined_df['period'] == 'Before Election'])}"
    )
    logging.info(
        f"After election: {len(combined_df[combined_df['period'] == 'After Election'])}"
    )
    ...
    for i, url in enumerate(urls_to_process):
        ...
        if (i + 1) % 100 == 0 or i == len(urls_to_process) - 1:
            ...
            if os.path.exists(output_path):
                ...
                logging.info(
                    f"Progress saved: {len(combined_df)} total rows in {output_path}"
                )
            else:
...
def generate_election_stats(sentiment_df, output_stats_path):
    ...
    if "period" not in sentiment_df.columns:
        election_year = (
            sentiment_df["election_year"].iloc[0]
            if "election_year" in sentiment_df.columns
            else 2020
        )
        ...
        sentiment_df.loc[sentiment_df["parsed_date"] >= election_date, "period"] = (
            "After Election"
        )

    # Calculate sentiment statistics
    sentiment_stats = (
        sentiment_df.groupby(["network", "period"])
        .agg(
    ...
    if tone_columns:
        tone_stats = (
            sentiment_df.groupby(["network", "period"])[tone_columns]
            .mean()
            .reset_index()
        )
        tone_stats.columns = [
            col if col in ["network", "period"] else f'Mean {col.replace("_score", "")}'
            for col in tone_stats.columns
        ]
    ...
    for network in sentiment_df["network"].unique():
        ...
        if len(network_data) >= 2:
            ...
            logging.info(
                f"  Before Election: {before['Count']} articles, {before['Positive_VADER']*100:.1f}% positive"
            )
            ...
            logging.info(
                f"  After Election: {after['Count']} articles, {after['Positive_VADER']*100:.1f}% positive"
            )
...
def compare_elections():
    ...
    if all_stats:
        ...
        combined_stats.to_csv(
            "../data/all_elections_sentiment_comparison.csv", index=False
        )
        ...
        for network in combined_stats["network"].unique():
            ...
            for year in sorted(network_data["election_year"].unique()):
                ...
                if len(year_data) >= 2:
                    ...
                    logging.info(
                        f"    Before: {before['Count']} articles, {before['Positive_VADER']*100:.1f}% positive"
                    )
                    logging.info(
                        f"    After: {after['Count']} articles, {after['Positive_VADER']*100:.1f}% positive"
                    )

                    if "Mean vader_tone" in before and "Mean vader_tone" in after:
                        tone_change = (
                            after["Mean vader_tone"] - before["Mean vader_tone"]
                        )
